...
class GlassMaterial extends THREE.ShaderMaterial {
	constructor(options = {}) {
		const uniforms = {
			...
			uRefractPower: { value: options.refraction || 1.2 },
			uChromaticAberration: { value: options.chromaticAberration || 0.15 },
			...
			uLight: { value: options.light || new THREE.Vector3(0.0, 1.0, 0.0) },
			uColor: {
				value: options.color
					? new THREE.Color(options.color)
					: new THREE.Color(0.008, 0.0, 0.106),
			},
			winResolution: {
	...
	static vertexShader() {
		return /*glsl*/ `
            ...
            // Compute projected coordinates (0 -> 1)
            vec4 projPos = projectionMatrix * mvPosition;
            vProjUV = projPos.xy / projPos.w * 0.5 + 0.5;
        }`
	}
		...
		return /*glsl*/ `
            ...
            void main() {
                ...
                // Apply base color tint with reduced influence
                 color = mix(color, color * uColor, 0.7);
	...
	renderPasses(renderer, scene, camera, meshes, renderBackside = false) {
